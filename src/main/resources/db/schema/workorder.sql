CREATE TABLE IF NOT EXISTS RSHM.TAB_WORKORDER (
    -- SYSTEM COLUMN
    ...
    etc_content LONGTEXT,  -- 특이사항

    charge_emp_id VARCHAR(255),     -- 업무 담당자
    plan_start_date DATETIME,     -- 예정 시작일시
    plan_finish_date DATETIME,     -- 예정 종료일시
    act_start_date DATETIME,     -- 실제 시작일시
    act_finish_date DATETIME,     -- 실제 종료일시

    vacation_cd VARCHAR(255),     -- 휴가 유형(Code Type: VACATION, 연차/반차(오전)/반차(오후)...)
    PRIMARY KEY (id)
) ENGINE=InnoDB;
